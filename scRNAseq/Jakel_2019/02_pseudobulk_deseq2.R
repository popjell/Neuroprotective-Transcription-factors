# Patient-level pseudobulk using the reclustered neurons.
library(Matrix)
library(DESeq2)
library(ggplot2)
...
counts <- x$counts
meta <- x$meta
clusters <- read.csv("results/neuron_clusters.csv")

# Keep the QC-passing neurons and attach their new Seurat cluster.
meta$subtype <- clusters$neuron_cluster[match(colnames(counts), clusters$cell)]
keep <- !is.na(meta$subtype)
meta <- meta[keep, ]
counts <- counts[, keep]

# Sum raw counts for every patient and new neuronal cluster.
...
sample_info <- meta[match(samples, group), c("patient", "Condition", "subtype")]
...
for (subtype in sort(unique(sample_info$subtype))) {
  ...
  if (sum(info$Condition == "Ctrl") < 2 || sum(info$Condition == "MS") < 2) next

  info$condition <- factor(info$Condition, levels = c("Ctrl", "MS"))
...
ggsave("plots/pseudobulk_pca.png", p, width = 10, height = 7, dpi = 200)
...
ggsave("plots/volcano.png", p, width = 10, height = 7, dpi = 200)

# Count significant Rank 1 and Rank 2 genes in each new neuronal cluster.
